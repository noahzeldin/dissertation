...
The choir sings for `r round(dur_choir_total_min, digits = 2)` minutes.
...
The choir sings for `r dur_choir_prop` % of total duration of music.
...
Only antiphonic passage at beginning of "Lob der Partei." Doesn't matter, because t1 and t2 overlap.
...
Generate basic table for whole work.
...
Relevel factors for piece_no. **CONSIDER COMBINING WITH ABOVE**
Necessary, so that bars in barcharts below appear in correct order.
...
Create mosaic plot. **FIX**

```{r, eval=FALSE}
# dur_texture_piece %>% 
#     ggplot(aes(fill = texture, y = prop_of_sung, x = piece_no, width = dur_sung)) +
#     geom_bar(position = "stack", stat = "identity", 
#              varwidth = TRUE # play with this
#              ) +
#     xlab("Piece Number") +
#     scale_fill_discrete(name = "Texture",
#                         labels = c("Antiphony", "Homophony", 
...
Attempt with ggmosaic package...fail...
**SKIPPING**

```{r, eval=FALSE}
ggplot(data = dur_texture_piece) +
    geom_mosaic(aes(x = product(texture, prop_of_sung),
                    fill = texture),
                na.rm = TRUE) +
    facet_grid(.~piece_no) +
    # coord_flip() +
...
# still a bit of tweaking - see explanation at bottom
dur_texture_piece_mosaic_plot <- ggplot(dur_texture_piece_mosaic_table, 
       aes(x = piece_no, y = prop_of_piece, width = dur_piece, fill = texture)) +
   ...
   # theme(panel.spacing.x = unit(0, "npc")) + # if no spacing preferred between bars
    ...
    theme(axis.title.y = element_blank(),
          ...
          strip.text = element_blank(), # does nothing
          strip.text.y = element_blank(), # does nothing
          strip.background.y = element_blank(), # does nothing
          panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_rect(fill = NA, color = "white"),
          rect = element_blank() # does nothing
          # panel.grid.major = element_blank(), # does nothing
          # panel.grid.minor = element_blank() # does nothing
    ) +
    ...
    # ggtitle("Proportion of Texture by Piece") 
# +    theme_void() # removes x-axis labels and includes facet labels - problematic b/c 13a is cut off

dur_texture_piece_mosaic_plot
```


#### Presence of each texture in each piece.
