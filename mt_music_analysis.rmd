...
Check to see if proportions add up to 1, in order to ensure that order of 
previous step is correct.

```{r}
sum(groupings_tib$prop_of_groupings) == 1
```

Create a basic barplot of groupings for whole work.  
...
groupings_tib %>% 
    ggplot() +
    geom_col(aes(x = groupings, y = prop_of_groupings, fill = groupings))
```


Create list of 5 most common groupings.
* **Try to fix final line of code - base R syntax, not tidyverse.**
...
Wow, really cool findings:   

* The grouping **st_ab** is more common than no groupings.  

* Only 18% of all measures with singing have no groupings.  


#### By duration (of measures)  

Generate table with groupings, duration and proportion.  
...
Only a little different from   
Top 3 and no. 5 are same as `groupings_5` but 4th place now **satb** (rather than st). 

##### And separated by Texture
...
groupings_separate_tib <- gen_tib_sung %>% 
    ...
    filter(groupings != "na") %>% 
    count(groupings) %>% 
    mutate(prop_of_groupings = (n/sum(n)*100)) 
```
...
Generate table with each separate grouping and its total duration.  
...
Pretty interesting - factoring in duration seems to make basically no 
difference.  
